...
public class NMEAParser implements Runnable {
    ...
    /**
     * Constructor for the NMEA parser. All information provided
     * is used to activate a GPS simulator. The data of the simulator
     * is parsed and its information is communicated through NMEAInfo
     * objects
     *
     * @param _file   The file to be used as input
     * @param _filter The filter stmt used to identify a new block
     * @param _delay  The amount of milliseconds the method blocks
    ...
    /**
     * Attaches a new Listener to the NMEA parser. Listeners
     * are provided with a new NMEA-0183 data block worth of
     * information (in form of a NMEAInfo object) as soon as
        ...
        try {
            while ((line = mGPSSim.readLine()) != null && mRUN) {
                ...
                if (checksum(payload, checksum)) {
                    ...
                    System.out.println("NMEAParser::run() checksum error in line (" + line + ") for payload [" + payload + "] and checksum [" + checksum + "]");
                }
            } // while line
        } catch (Exception _e) {
    ...
    /**
     * Checks if the XOR combined bytes of _data are
     * corresponding to the provided HEX _checksum
     *
    ...
    /**
     * Method that parses GSV data sentences. Typically multiple
     * lines with the same talker id are available.
     *
        ...
        for (int i = 3; i < _data.length; i = i + 4) {
            if (_data.length > i + 2) { // sentence contains data
                ...
                switch (_talkerId) {
                    ...
                    break;
                    default:
                        System.out.println("NMEAParser::parseGSV unhandled talker ID encountered: " + _talkerId);
                } // switch talkerId
    ...
    /**
     * Method that parses GSA data sentences. Typically multiple
     * lines with the same talker id are available.
     *
     * @param _talkerId The id of the information provider (GPS, Galileo, GLONASS, ...)
