...
public class SatView extends Pane implements INMEAUpdate {
  ...
  protected void drawSatellite(List<ASatelliteInfo> _sats) {
    ...
    gc.strokeOval(centerX - (diameter45 >> 1), centerY - (diameter45 >> 1), diameter45, diameter45);
    // 2. Lines
    gc.strokeLine(
            centerX - (radius + (PADDING >> 1)), centerY, centerX + (radius + (PADDING >> 1)), centerY);
    gc.strokeLine(
            centerX, centerY - (radius + (PADDING >> 1)), centerX, centerY + (radius + (PADDING >> 1)));
    // 3. Text
