...
/**
 * Class that manages the drawing of GPS satellites
 * on the satellite view (SatView)
 * <p>
 * Satellites are represented by filled circles and white text/border
 *
 * @author jkroesche
 * @see SatView
 */
public class GPS_SatelliteInfo extends ASatelliteInfo {
    ...
    protected void drawSatellite(GraphicsContext _gc, int _x, int _y) {
        _gc.fillOval(_x - (RADIUS >> 1),
                _y - (RADIUS >> 1),
                RADIUS,
                RADIUS);
        _gc.setStroke(Color.WHITE);
        _gc.strokeOval(_x - (RADIUS >> 1),
                _y - (RADIUS >> 1),
                RADIUS,
                RADIUS);
